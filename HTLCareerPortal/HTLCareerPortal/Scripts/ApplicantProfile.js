...
function Profle() {
    ...
    $.ajax({
        url: "/Admin/Home/ProfessionalReference",
        type: "GET",
        contentType: "application/json;charset=utf-8",
        datatype: "json",
        success: function (result) {
    ...
    $.ajax({
        url: "/Admin/Home/CareerObjective",
        type: "GET",
        contentType: "application/json;charset=utf-8",
        datatype: "json",
        success: function (result) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if ($.trim(result)) {
                var html = '';
                $.each(result, function (key, item) { 
                    html +=  '<a  href="/Resumes/'+item.ResumeUrl+'" download>Download Resume<span class="glyphicon glyphicon-download"></span></a>';
                });
                $('.Resume-Detail').html(html);
            }
            else
                $('.Resume-Detail-Section').hide();
        },
        error: function (errormessage) {
            alert(errormessage.responseText);
        }
